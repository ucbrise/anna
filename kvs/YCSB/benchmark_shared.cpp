...
typedef Concurrent_KV_Store<int, KVS_PairLattice<MaxLattice<int>>> Shared_Database;
...
void *worker_routine (zmq::context_t *context, int thread_id, put_request *request, Shared_Database *shared_kvs)
{
    ...
    for (int i = 0; i < REQUEST_NUM; i++) {
        ...
        if (update_counter == THRESHOLD) {
            ...
            for (auto it = change_set->reveal().begin(); it != change_set->reveal().end(); it++) {
                shared_kvs->put(*it, kvs->get(*it));
                c_data->keys.insert(*it);
            }
        ...
        if ((rc = zmq_msg_recv(&rec, static_cast<void *>(subscriber), ZMQ_DONTWAIT)) != -1) {
            ...
            for (auto it = c_data->keys.begin(); it != c_data->keys.end(); it++) {
                kvs->put(*it, shared_kvs->get(*it));
            }
...
int main ()
{
    ...
    // Create shared kvs
    Shared_Database *shared_kvs = new Shared_Database;
    ...
    for (int thread_id = 0; thread_id != THREAD_NUM; thread_id++) {
        threads.push_back(thread(worker_routine, &context, thread_id, request[thread_id], shared_kvs));
    }
